...
class TestCase(Classes):
    ...
    def to_string(self, whitespace: bool=False):
        ...
        if (self.functionArguments != None) and (type(self.functionArguments)==type(str(""))): out = f"{out}{white2}, functionArguments=\"{self.functionArguments}\""
        if (self.functionArguments != None) and (type(self.functionArguments)!=type(str(""))): out = f"{out}{white2}, functionArguments={self.functionArguments}"
        if self.functionKeyWordArguments != None: out = f"{out}{white2}, functionKeyWordArguments={self.functionKeyWordArguments}"
        if (self.expectedOutputValueOf != None) and (type(self.expectedOutputValueOf)==type(str(""))): out = f"{out}{white2}, expectedOutputValueOf=\"{self.expectedOutputValueOf}\""
        if (self.expectedOutputValueOf != None) and (type(self.expectedOutputValueOf)!=type(str(""))): out = f"{out}{white2}, expectedOutputValueOf={self.expectedOutputValueOf}"
        if self.expectedStdOutputValueOf != None: out = f"{out}{white2}, expectedStdOutputValueOf=\"{self.expectedStdOutputValueOf}\""
        if self.expectedException != None: out = f"{out}{white2}, expectedException={self.expectedException}"
        if self.stdInInput != None: out = f"{out}{white2}, stdInInput=\"{self.stdInInput}\""
        out = f"{out})"
        return out
